...
def test_get_pois_from_polygon(polygon_area):
    pois_object = Pois()
    pois_object.from_polygon(polygon_area)

    assert isinstance(pois_object.geo_filter, PolygonArea)
    assert 'poly:' in pois_object.geo_filter.location_name
    assert "node" in pois_object.query
    assert len(pois_object.data) > 0
    assert isinstance(pois_object.data, list)
    assert isinstance(pois_object.data[0], dict)
...
def test_get_vehicle_network_from_polygon_area(vehicle_mode, polygon_area):
    roads_object = Roads(vehicle_mode)
    roads_object.from_polygon(polygon_area)

    assert isinstance(roads_object.geo_filter, PolygonArea)
